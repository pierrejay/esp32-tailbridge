...
cleanup_esp() {
    ...
    iptables -D FORWARD -i wg0 -o veth-h-$name -j ACCEPT 2>/dev/null
    iptables -D FORWARD -i veth-h-$name -o wg0 -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null
    iptables -D FORWARD -i ens3 -o veth-h-$name -j ACCEPT 2>/dev/null
    iptables -D FORWARD -i veth-h-$name -o ens3 -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null
